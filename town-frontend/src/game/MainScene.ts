...
export class MainScene {
  ...
  private setupCanvasClick(): void {
    this.renderer.domElement.addEventListener('click', (e) => {
      ...
      for (const npc of this.npcManager.getAll()) {
        ...
        // If click is within 2.5 units of the NPC's projected position on ray
        // (increased from 1.5 to account for NPCs near buildings that block raycast)
        if (distToRay < 2.5) {
  ...
  onNpcPhase(npcId: string, phase: PlayerPhase): void {
    ...
    if (npc) {
      ...
      // 进入 punishing 阶段时立即暂停漫游
      // 防止 NPC 继续移动导致管理员走向旧位置
      if (phase === 'punishing') {
  ...
  onAlertPopup(playerId: string, cheatType: CheatType, confidence: Confidence, message: string, npcId?: string): void {
    ...
    // 首次警告：在 NPC 上显示警告指示器
    if (message.includes('首次警告')) {
      const npc = this.npcManager.get(playerId)
      if (npc) {
        npc.showWarning(cheatType)
      }
    }
  }
  ...
  onPhaseChange(playerId: string, oldPhase: PlayerPhase, newPhase: PlayerPhase, reason: string, vpTotal: number, cheatType?: CheatType, npcId?: string): void {
    ...
    // 解封：从 punishing/offline 恢复到 normal/monitoring 时，平滑移动回广场
    if ((oldPhase === 'punishing' || oldPhase === 'offline') && (newPhase === 'normal' || newPhase === 'monitoring')) {
      const npc = this.npcManager.get(effectiveNpcId)
      if (npc && npc.detained) {
        // 查找 NPC 当前所在建筑的门位置
        const npcPos = npc.getPosition()
        const buildingId = this.townBuilder.isInsideBuilding(npcPos.x, npcPos.z, 3.0)
        if (buildingId && this.townManager) {
          const doorMarker = this.townBuilder.getDoorMarker(buildingId)
          if (doorMarker) {
            const doorPos = { x: doorMarker.position.x, z: doorMarker.position.z }
            // 暂停漫游
            this.npcManager.pauseRoaming(effectiveNpcId)
            // 先移动到门位置（等待完成后再开始押送）
            npc.setCollisionEnabled(false)
            npc.moveTo({ x: doorPos.x, z: doorPos.z }, 2.5).then(() => {
              npc.setVisible(true)
              npc.restoreVisual()
              // 管理员押送回广场（管理员会走到门附近而非门正上方，避免重叠）
              this.townManager!.escortNpcFromBuilding(effectiveNpcId, doorPos).then(() => {
                // 押送完成，NPC 到达广场中心附近，确保不与其他 NPC 重叠
                const finalPos = npc.getPosition()
                const safePos = this.npcManager.findNonOverlappingPosition(finalPos.x, finalPos.z, effectiveNpcId)
                if (safePos.x !== finalPos.x || safePos.z !== finalPos.z) {
                  npc.moveTo({ x: safePos.x, z: safePos.z }, 2.5).then(() => {
        ...
        // 更新关押区状态
        if (buildingId) {
          const playerName = info?.name ?? effectiveNpcId
          this.townBuilder.removeDetainedPlayer(buildingId, playerName)
        }
      }
    }
  }
  ...
  onPenaltyEvent(data: { playerId: string; npcId?: string; level: string; action: string; cheatType: CheatType; confidence: Confidence; vp: number; reason: string; autoGenerated: boolean; duration?: string; bannedAt?: number }): void {
    ...
    const statusLabel = actionStatusMap[data.action] ?? data.action
    const toast = document.createElement('div')
    toast.className = `penalty-toast penalty-toast-${data.level === 'L0' ? 'warning' : 'error'}`
    toast.innerHTML = `<span class="penalty-status">${statusLabel}</span> ${message}`
    document.body.appendChild(toast)
    ...
    if ((data.action === 'ban' || data.action === 'kick_ban') && info) {
  ...
  private startDetentionRoaming(npcId: string, doorX: number, doorZ: number, buildingKey?: string): void {
    ...
    const roam = () => {
      ...
      const dist = 1.5 + Math.random() * (RADIUS - 1.5)  // 1.5 ~ 3.5 距离（远离建筑边缘）
      // 旋转门朝向角度
      ...
      // NPC 间碰撞检查：如果目标点与其他 NPC 太近，使用非重叠位置
      const safePos = this.npcManager.findNonOverlappingPosition(targetX, targetZ, npcId)

      n.moveTo({ x: safePos.x, z: safePos.z }, 0.8).then(() => {
        // 到达后等待 2-5 秒，继续漫游
  ...
  onNpcBuildingLeave(npcId: string, buildingKey: string, actualStayMs: number): void {
    ...
    // 管理员押送 NPC 从关押区回到广场中心
    if (doorMarker && this.townManager) {
      const doorPos = { x: doorMarker.position.x, z: doorMarker.position.z }
      // 暂停漫游，等待押送完成
      this.npcManager.pauseRoaming(npcId)
      this.townManager.escortNpcFromBuilding(npcId, doorPos).then(() => {
        // 押送完成，确保不与其他 NPC 重叠
        const finalPos = npc.getPosition()
        const safePos = this.npcManager.findNonOverlappingPosition(finalPos.x, finalPos.z, npcId)
        if (safePos.x !== finalPos.x || safePos.z !== finalPos.z) {
          npc.moveTo({ x: safePos.x, z: safePos.z }, 2.5).then(() => {
