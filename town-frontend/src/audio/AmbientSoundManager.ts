/**
 * AmbientSoundManager — 程序化环境音合成器
 * 零音频文件依赖，使用 Web Audio API 噪声生成器 + 振荡器 + 滤波器
 * 生成雨声、风声、鸟鸣、虫鸣、交通声、雷声
 */

type WeatherType = 'clear' | 'rain' | 'snow' | 'sandstorm' | 'fog' | 'aurora' | 'storm' | 'heavyRain' | 'drizzle' | 'cloudy' | 'blizzard' | 'lightSnow'
...
export class AmbientSoundManager {
  ...
  playThunder(intensity: number): void {
    ...
    for (let i = 0; i < segments; i++) {
      ...
      env.gain.linearRampToValueAtTime(0.01, now + offset + segDuration)
