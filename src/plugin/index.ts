...
import { DetectionEngine, type RecentData, initSpeedThresholdService, shutdownSpeedThresholdService } from './rule-engine.js'
...
async function main(): Promise<void> {
  ...
  const detectionEngine = new DetectionEngine(3000)
  ...
  function runDetection(playerId: string): void {
    ...
    if (detections.length > 0) {
      for (const detection of detections) {
        ...
        // 更新 Phase（PenaltyEngine 为单一真相源，通过 MonitorBridge 同步）
        ...
        const record = {
          ...
          confidence: detection.confidence,
          evidence: detection.evidence,
          action: penaltyResult.triggered ? `auto_${penaltyResult.level}` : 'detect',
          actionResult: penaltyResult.triggered ? 'penalty_dispatched' : 'recorded',
        ...
        if (penaltyResult.triggered && penaltyResult.action) {
          ...
          // 同步 BanManager — ban 或 kick（kick 时因 supplementaryActions 包含 ban，也需同步）
          if (penaltyResult.action.type === 'ban') {
            ...
            // kick 时 supplementaryActions 包含临时封禁，同步到 BanManager
